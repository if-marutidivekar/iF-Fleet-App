...
export function EmployeeProfilePage() {
  ...
  const saveProfile = useMutation({
    ...
    onError: (e: unknown) => {
      ...
      setEditError(Array.isArray(msg) ? msg.join(', ') : (msg ?? 'Failed to save. Please try again.'));
    },
  });
  ...
  const initials = (user.firstName ? user.firstName[0] : user.name[0] ?? 'E').toUpperCase();
          ...
          <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 12 }}>
            {!editing ? (
              <>
                ...
                <button
                  onClick={startEdit}
                  style={{ padding: '9px', background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 8, color: '#2563eb', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}
                >
                  ✏ Edit Profile
                </button>
              </>
              ...
              <form onSubmit={(e) => { e.preventDefault(); saveProfile.mutate(); }} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <label style={{ fontSize: 13, fontWeight: 600, color: '#374151', display: 'flex', flexDirection: 'column', gap: 4 }}>
                  Department *
                  <input type="text" value={department} onChange={(e) => setDepartment(e.target.value)} required style={inputStyle} placeholder="Engineering" />
                </label>
                <label style={{ fontSize: 13, fontWeight: 600, color: '#374151', display: 'flex', flexDirection: 'column', gap: 4 }}>
                  Mobile Number
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                  <button
                    ...
                    style={{ flex: 1, padding: '9px', background: '#2563eb', color: '#fff', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: 'pointer', opacity: saveProfile.isPending ? 0.7 : 1 }}
                  >

...
export default function EmployeeHistory() {
  ...
  const { data: bookings = [], isLoading, refetch } = useQuery<Booking[]>({
    ...
    queryFn: () => api.get<Booking[]>('/bookings').then(r =>
      r.data.filter(b => DONE.includes(b.status)).sort((a, b) => new Date(b.requestedAt).getTime() - new Date(a.requestedAt).getTime())
    ),
  ...
  return (
    ...
    >
      <View style={[s.header, { paddingTop: insets.top + 16 }]}>
        ...
        {bookings.length > 0 && <Text style={s.count}>{bookings.length} trips</Text>}
      </View>
      ...
      {!isLoading && bookings.length === 0 && (
        <View style={s.empty}>
          ...
          <Text style={s.emptyText}>Completed and past bookings will appear here</Text>
        </View>
      )}
        ...
        {bookings.map(b => {
          ...
          return (
            <View key={b.id} style={[s.card, b.status === 'COMPLETED' ? {} : { opacity: 0.7 }]}>
              ...
              {km !== null && <Text style={s.meta}>📏 {km} km travelled</Text>}
              {trip?.actualEndAt && (
                <Text style={s.meta}>✓ Completed {new Date(trip.actualEndAt).toLocaleDateString()}</Text>
              )}
            </View>
          );
        })}
